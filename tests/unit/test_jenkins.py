...
class TestJenkins:
    ...
    def test_move(self, jenkins, respx_mock):
        headers = {
            'Location': f'{jenkins.url}job/Level1_Folder2/job/Level1_Folder1/'}
        req_url = f'{jenkins["Level1_Folder1"].url}move/move'
        respx_mock.post(req_url).respond(headers=headers)
        jenkins.move_job('Level1_Folder1', 'Level1_Folder2')
        assert respx_mock.calls[0].request.url == req_url

    def test_rename(self, jenkins, respx_mock):
        headers = {'Location': f'{jenkins.url}job/Level1_Folder3/'}
        req_url = f'{jenkins["Level1_Folder1"].url}confirmRename?newName=Level1_Folder3'
        respx_mock.post(req_url).respond(headers=headers)
        jenkins.rename_job('Level1_Folder1', 'Level1_Folder3')
        assert respx_mock.calls[0].request.url == req_url
...
class TestAsyncJenkins:
    ...
    @pytest.mark.parametrize('name,type_', [('not exist', type(None)),
                                            ('Level1_Folder1', AsyncFolder),
                                            ('Level1_Folder1/Level2_Folder1',
                                             AsyncFolder),
                                            ('http://0.0.0.0:8080/job/Level1_Folder1/job/Level2_Folder1', AsyncFolder),
                                            ('Level1_Folder1/Level2_WorkflowJob',
                                             AsyncWorkflowJob),
                                            ('Level1_Folder1/not exist', type(None))])
    ...
    async def test_create_job_fail(self, async_jenkins, respx_mock, headers):
        ...
        with pytest.raises(BadRequestError, match=r'exists|unsafe'):
            await async_jenkins.create_job('Level1_Folder1', '')
    ...
    async def test_copy_fail(self, async_jenkins, respx_mock, headers):
        respx_mock.post(f'{async_jenkins.url}createItem').respond(
            headers=headers, status_code=400)
        with pytest.raises(BadRequestError):
            await async_jenkins.copy_job('not exist', 'Level2_Folder1')
    ...
    async def test_delete_job(self, async_jenkins, respx_mock):
        req_url = f'{async_jenkins.url}job/Level1_Folder1/doDelete'
        respx_mock.post(req_url)
        await async_jenkins.delete_job('Level1_Folder1')
        assert respx_mock.calls[0].response.status_code == 200
        assert respx_mock.calls[0].request.url == req_url

    @pytest.mark.parametrize('name, exception', [('not exist', ItemNotFoundError),
                                                 ('Level1_Folder1', TypeError)])
    ...
    async def test_build_job_succ(self, async_jenkins, respx_mock, name, entry, params):
        ...
        await async_jenkins.build_job(name, **params)
        assert respx_mock.calls[0].request.url == req_url
    ...
    async def test_iter_jobs(self, async_jenkins):
        assert len([j async for j in async_jenkins.iter_jobs()]) == 2
        assert len([j async for j in async_jenkins]) == 2

    def test_no_class_for_item(self, async_jenkins):
        with pytest.raises(AttributeError) as e:
